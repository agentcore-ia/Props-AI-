        ...
        <div className="p-6">
          ...
          {success ? (
            <div className="mt-5 rounded-3xl border bg-muted/40 p-4">
              ...
              <p className="mt-2 text-sm text-muted-foreground">Acceso: {success.email}</p>
              <p className="mt-1 text-sm text-muted-foreground">Password: {success.password}</p>
              <p className="mt-1 text-sm text-muted-foreground">Portafolio: {success.slug}.props.com.ar</p>
            </div>
          ) : null}
        </div>
        <DialogFooter>
...
export function AgencyOverviewCards({ agencies }: { agencies: AgencySummary[] }) {
...
export function AgencyList({ agencies }: { agencies: AgencySummary[] }) {
  return (
    ...
    <div className="space-y-8">
      {showHeader ? (
        <PageHeader
          ...
          description="Crea clientes nuevos, entrega el acceso inicial y controla la vidriera publica de cada inmobiliaria desde un solo lugar."
          action={<CreateAgencyDialog />}
        />
      ) : null}

      <AgencyOverviewCards agencies={agencies} />
