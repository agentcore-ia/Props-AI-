...
export function buildPublicListings(
  ...
  return properties.map((property, index) => {
    ...
    const bathrooms = Math.max(0, Number(property.bathrooms ?? 0));
    const suites = property.bedrooms > 1 && property.bathrooms > 1 ? 1 : 0;
    const area = Math.max(0, Number(property.area ?? 0));
    const lotArea = area > 0 ? area + Math.max(0, property.parkingSpots) * 12 : 0;
    ...
    const pricePerSquareMeter = area > 0 ? Math.round(property.price / area) : 0;
